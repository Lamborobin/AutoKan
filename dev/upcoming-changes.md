...
## Not Yet Built
  ...
  **Generic action hooks for System Behavior** — this layer is documented as able to invoke code-exposed actions/hooks that are capability-agnostic (e.g. "at 100 units → call the email endpoint"), but no such mechanism actually exists — there's no registry connecting its text to a real function call. `notify_all` is the closest thing built so far, and it's deliberately capability-scoped (wired only into the Planner's tool set via `CLARIFICATION_TOOLS`), not the generic version. Tested directly: placing the same `notify_all` trigger text in System Rules did fire it — but only because it happens to route through the Planner, which already has that tool. Confirms the gap rather than closing it. Building the real thing means a registry mapping named actions to functions, decided as available to *any* capability rather than requiring the per-capability tool-schema wiring every new capability-scoped tool needs today — and almost certainly a permissions/allow-list question (which actions a given install is allowed to expose at all) before it's safe to let a superadmin's free-text rule trigger a real side effect.
