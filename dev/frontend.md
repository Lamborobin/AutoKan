...
**System**:
- **System Behavior** — edits the governance docs surfaced from `agent.config.json`'s `ai_context` groups (component: `AiContextPanel`); always manual-save with a confirm dialog, and versioned server-side. Never had auto-save to begin with.
