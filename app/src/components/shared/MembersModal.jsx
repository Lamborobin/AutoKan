...
function RolePill({ color, label }) {
  return (
    <span className="inline-flex items-center px-1.5 py-0.5 rounded-md text-xs font-medium shrink-0"
...
export default function MembersModal({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" data-modal-backdrop="static">
      <div className="bg-surface-1 border border-border rounded-xl w-full max-w-lg shadow-xl overflow-hidden max-h-[85vh] flex flex-col">
          ...
          {[['members', 'Board Members'], ['teams', 'Teams']].map(([id, label]) => (
            <button key={id} onClick={() => setActiveTab(id)}
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                activeTab === id ? 'text-accent border-b-2 border-accent' : 'text-gray-500 hover:text-gray-100'
              }`}
            >
              {label}
              {id === 'members' && boardMembers.length > 0 && <span className="ml-1.5 text-sm text-gray-600">{boardMembers.length}</span>}
              {id === 'teams'   && teams.length > 0           && <span className="ml-1.5 text-sm text-gray-600">{teams.length}</span>}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'members' && (
            <div className="divide-y divide-border">
                      ...
                      <div className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-surface-3 transition-colors group">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {isSelf && <span className="text-xs font-medium px-1.5 py-0.5 rounded-full bg-accent/15 text-accent border border-accent/25 uppercase tracking-wide shrink-0">You</span>}
                            {!member.accepted_at && <span className="text-xs font-medium px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/25 uppercase tracking-wide shrink-0">Pending</span>}
                            {isSuperAdmin
                              ? <span className="inline-flex items-center gap-0.5 text-xs font-medium px-1.5 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/25 uppercase tracking-wide shrink-0"><Shield size={8} /> Super Admin</span>
                              : memberRoles.slice(0, 3).map(r => <RolePill key={r.id} color={r.color} label={r.name} />)
                            }
                            {!isSuperAdmin && memberRoles.length > 3 && (
                              <span className="text-xs text-gray-500">+{memberRoles.length - 3}</span>
                            )}
                          </div>
                          {name && <p className="text-sm text-gray-500 truncate">{member.email}</p>}
                        </div>
                      ...
                      {isEditing && !isSuperAdmin && (
                        <div className="mx-3 mb-2 bg-surface-2 border border-border rounded-xl p-3 space-y-3">
                          ...
                          {columnRoles.length > 0 && (
                            <div>
                              <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Column Access</p>
                              <div className="space-y-1">
                                {columnRoles.filter(r => r.id === 'role_access_any').map(role => (
                                  <label key={role.id} className="flex items-center gap-2 cursor-pointer group/r">
                                    ...
                                    <span className="text-xs text-gray-600 ml-auto">All columns</span>
                                  </label>
                                ))}
                                ...
                                {columnRoles.filter(r => r.id !== 'role_access_any').map(role => (
                                  <label key={role.id} className={`flex items-center gap-2 cursor-pointer group/r ${allColumnsChecked(editRoleIds) ? 'opacity-40 pointer-events-none' : ''}`}>
                                    ...
                                    <span className="text-sm text-gray-300">{role.name}</span>
                                    <span className="w-2.5 h-2.5 rounded-md ml-auto shrink-0" style={{ background: role.color }} />
                                  </label>
                                ))}
                              </div>
                            </div>
                          ...
                          {permRoles.length > 0 && (
                            <div>
                              <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Permissions</p>
                              <div className="space-y-1">
                                {permRoles.map(role => (
                                  <label key={role.id} className="flex items-center gap-2 cursor-pointer group/r">
                                    <div
                          ...
                          <div className="flex gap-1.5 pt-1">
                            <button
                              ...
                              className="flex-1 py-1 text-sm text-gray-500 hover:text-gray-100 border border-border rounded-lg hover:bg-surface-3 transition-colors"
                            >
                              Cancel
                            </button>
                            <button
                              ...
                              className="flex-1 py-1 text-sm font-medium text-white bg-accent hover:bg-accent/80 rounded-lg disabled:opacity-40 transition-colors"
                            >
              ...
              {addableUsers.length > 0 && (
                <div className="p-4">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-2">Add to board</p>
                      ...
                      return (
                        <div key={u.id} className="flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-surface-3 transition-colors group">
                          ...
                          <button
                            ...
                            className="opacity-0 group-hover:opacity-100 flex items-center gap-1 px-2.5 py-1 text-sm font-medium text-accent bg-accent/10 hover:bg-accent/20 rounded-lg transition-all disabled:opacity-40 shrink-0"
                          >
              ...
              <div className="p-4">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-2">Invite by email</p>
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2 px-3 py-2 bg-green-500/10 border border-green-500/20 rounded-lg">
                    ...
                    {copiedLink && (
                      <button onClick={() => navigator.clipboard.writeText(copiedLink)}
                        className="shrink-0 p-1 rounded-md text-green-400 hover:bg-green-500/20 transition-colors" title="Copy invite link">
                        <Copy size={12} />
                      </button>
                    )}
                  </div>
            ...
            <div className="p-4 space-y-2">
                    ...
                    <div className="flex items-center">
                      ...
                      {confirmDeleteTeam === team.id ? (
                        <div className="flex items-center gap-1 px-2 shrink-0">
                          <span className="text-xs text-amber-400 whitespace-nowrap">Archive ({team.member_count})?</span>
                          <button onClick={() => handleDeleteTeam(team)} className="px-1.5 py-0.5 text-xs font-medium text-red-400 hover:bg-red-500/10 rounded-md transition-colors">Yes</button>
                          <button onClick={() => setConfirmDeleteTeam(null)} className="px-1.5 py-0.5 text-xs text-gray-400 hover:text-gray-200 rounded-md transition-colors">No</button>
                        </div>
                      ) : (
                    ...
                    {expandedTeam === team.id && (
                      <div className="border-t border-border bg-surface-2">
                        <div className="p-2 space-y-0.5">
                              ...
                              <div key={tm.email} className="flex items-center gap-2 px-2 py-2 rounded-lg hover:bg-surface-3 group transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    {isSelf && <span className="text-xs font-medium px-1 py-0.5 rounded-full bg-accent/15 text-accent border border-accent/25 uppercase tracking-wide shrink-0">You</span>}
                                    {!tm.user_id && <span className="text-xs font-medium px-1 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/25 uppercase tracking-wide shrink-0">Pending</span>}
                                  </div>
                          ...
                          {teamError[team.id] && (
                            <p className="text-xs text-red-400 px-1">{teamError[team.id]}</p>
                          )}
                        </div>
                      </div>
                    )}
              ...
              {showNewTeamForm ? (
                <form onSubmit={handleCreateTeam} className="border border-border rounded-xl p-3 space-y-2">
                  <p className="text-sm font-medium text-gray-400">New team</p>
                  ...
                  <div className="flex gap-1.5">
                    <button type="button" onClick={() => setShowNewTeamForm(false)}
                      className="flex-1 py-1.5 text-sm text-gray-500 hover:text-gray-100 border border-border rounded-lg hover:bg-surface-3 transition-colors">Cancel</button>
